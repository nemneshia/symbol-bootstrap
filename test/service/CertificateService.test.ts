...
describe('CertificateService', () => {
  ...
  it('create and renew certificates', async () => {
    ...
    expect(await service.run({ ...presetData, nodeCertificateExpirationInDays: 10 }, name, keys, RenewMode.WHEN_REQUIRED, target)).eq(true);
    await verifyCertFolder();
    ...
    expect(await service.run({ ...presetData, certificateExpirationWarningInDays: 11 }, name, keys, RenewMode.WHEN_REQUIRED, target)).eq(
      true,
    );
    await verifyCertFolder();
    expect(await getCertFile(CertificateService.CA_CERTIFICATE_FILE_NAME)).eq(originalCaFile);
  ...
  it('create and renew certificates always', async () => {
    ...
    //Renew is not required
    expect(await service.run({ ...presetData, certificateExpirationWarningInDays: 50 }, name, keys, RenewMode.WHEN_REQUIRED, target)).eq(
      false,
    );
    await verifyCertFolder();
    expect(await getCertFile(CertificateService.CA_CERTIFICATE_FILE_NAME)).eq(originalCaFile);
