...
export default class Encrypt extends Command {
  static description = `指定したパスワードで yaml ファイルを暗号化します。
  対象は custom preset、preset.yaml、addresses.yaml です。主な用途は custom preset の暗号化です。
  custom preset に秘密鍵が含まれる場合は、暗号化して Bootstrap の起動や設定時に --password を指定して利用することを強く推奨します。`;
...
$ symbol-bootstrap start --password 1234 --preset testnet --assembly dual --customPreset encrypted-custom-preset.yaml`,
  ...
  static flags = {
    ...
    source: Flags.string({
      char: 's',
      description: `暗号化する元の平文 yaml ファイルを指定します。すでに暗号化済みのファイルを指定するとエラーになります。`,
      required: true,
    }),
    ...
    password: CommandUtils.getPasswordFlag(
      `元ファイルを暗号化して出力先ファイルを作成する際に使うパスワードを指定します。
